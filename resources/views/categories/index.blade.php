...
<script type="text/javascript">
    ...
    var ref = database.collection('categoria').orderBy('nombreCat');
            ...
            ajax: function (data, callback, settings) {
                ...
                ref.get().then(async function (querySnapshot) {
                    ...
                    await Promise.all(querySnapshot.docs.map(async (doc) => {
                        ...
                        if (childData.id) {
                            childData.totalProducts = await getProductTotal(childData.nombreCat);
                        }
                        else {
                        ...
                        if (searchValue) {
                            if (
                                (childData.nombreCat && childData.nombreCat.toString().toLowerCase().includes(searchValue)) ||
                    ...
                    filteredRecords.slice(start, start + length).forEach(function (childData) {
                        ...
                        records.push([
                            ...
                            childData.imagenCat == '' || childData.imagenCat == null ? '<img class="rounded" style="width:50px" src="' + placeholderImage + '" alt="image">' : '<img onerror="this.onerror=null;this.src=\'' + placeholderImage + '\'" class="rounded" style="width:50px" src="' + childData.imagenCat + '" alt="image">',
                            '<a href="' + route1 + '">' + childData.nombreCat + '</a>',
    ...
    async function getProductTotal(id, section_id) {
        var vendor_products = database.collection('subCategoria').where('categoria', '==', id);
        var Product_total = 0;
        if (section_id) {
    ...
    $(document).on("click", "a[name='category-delete']", function (e) {
        var id = this.id;
        database.collection('categoria').doc(id).delete().then(function (result) {
    ...
    */
    ...
    $(document).on("click", "a[name='category-delete']", async function (e) {
        ...
        try {            
            var doc = await database.collection('categoria').doc(id).get();
            if (doc.exists) {
                var data = doc.data();
                var catImageFile = data.imagenCat;
                if (catImageFile) {                    
                    try {
                        ...
                        if (imageBucket == envBucket) {
                            ...
                            console.log("El bucket no coincide, no se elimina la imagen.");
                        }
                    } catch (error) {
        ...
        if ($('#categoriesTable .is_open:checked').length) {
            if (confirm("{{trans('lang.selected_delete_alert')}}")) {
                ...
                $('#categoriesTable .is_open:checked').each(function () {
                    ...
                    database.collection('categoria').doc(dataId).delete().then(function () {
                        setTimeout(function () {
    ...
    $(document).on("click", "input[name='isActive']", function (e) {
        ...
        var id = this.id;
        if (ischeck) {
            database.collection('categoria').doc(id).update({'publish': true}).then(function (result) {
            });
        } else {
            database.collection('categoria').doc(id).update({'publish': false}).then(function (result) {
            });
        }
    });
</script>
